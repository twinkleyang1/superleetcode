...
- **自适应遗忘算法**：根据遗忘比例、遗忘总次数、距上次遗忘时间计算 forgetScore，动态调整每日复习目标（1-4 次），同日多次遗忘可重新入队
- **掌握评级降级**：同日首次"完全掌握"与挣扎后"完全掌握"区别对待，通过 7 因子置信度评分（遗忘次数、总评次数、间隔天数、历史遗忘分、当前等级、震荡趋势、题目难度）自动降级有效等级
- **每日复习队列**：今日到期的题目高亮提醒，遗忘分排序 + 进度比例二次排序
- **数据统计**：刷题进度、掌握趋势图表（按日去重统计）
